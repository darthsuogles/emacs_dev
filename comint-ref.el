...
;;!+TODO: command string sent to the inferior process for completion info
...
;; -------------------------------------------------------
;; Torch7
(defun th-send-string (string &optional process)
  "Evaluate STRING in Torch TREPL (TH) process."
  (interactive "sPython command: ")
  (let* ((proc (or process 
                   (or torch-process
                       (get-buffer-process torch-process-buffer))))
         (buffer (or torch-process-buffer (process-buffer proc))))
    (with-current-buffer buffer
      (goto-char (point-max))
      (unless (string-match "\\`" string)
        (comint-send-string proc "\n"))
...
(defun th--send-string-return-output (string &optional process msg)
...
(th--send-string-return-output "nnet:\t")
...
;;!+TODO: this is the actual place where code is sent and values retrieved
...
;;!+TODO: ipython's shell completion
...
;;!+TODO: the indent completion part, check this out!!
...
;;!+TODO: interaction with the inferior shell
